...
| [`ferromotion-core`](https://crates.io/crates/ferromotion-core) | FK, analytic + multi-frame Jacobians, **RNEA** inverse dynamics + **ABA** O(n) forward dynamics + **floating-base** dynamics, mass matrix, IK (LM + robust), trajectory optimization, sparse factor-graph solve, collision costs, motion retargeting, augmented-Lagrangian, **differentiable contact** (interior-point LCP + articulated frictional contact on a robot chain), URDF loading. |
