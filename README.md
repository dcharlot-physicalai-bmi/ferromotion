...
| [`ferromotion-core`](https://crates.io/crates/ferromotion-core) | FK, analytic + multi-frame Jacobians, **RNEA** inverse dynamics + **ABA** O(n) forward dynamics + **floating-base** dynamics, mass matrix, IK (LM + robust), trajectory optimization, sparse factor-graph solve, collision costs, motion retargeting, augmented-Lagrangian, **differentiable contact** (interior-point LCP + articulated frictional contact + contacts-from-distance gradients), **closed-loop / parallel mechanisms** (loop-closure KKT dynamics), URDF loading. |
| [`ferromotion-control`](https://crates.io/crates/ferromotion-control) | PID · computed-torque · Cartesian impedance · LQR · linear MPC · OSC · WBC · placo · iLQR/DDP · **constrained DDP (PROXDDP)** · MPPI · CEM · CBF-QP · sliding-mode · **SRBD MPC** · capture-point/ZMP · centroidal MPC · Kalman/EKF/UKF · complementary filter · momentum observer. |
