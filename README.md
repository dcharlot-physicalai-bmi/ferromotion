...
| [`ferromotion-fem`](https://crates.io/crates/ferromotion-fem) | Differentiable volumetric **tetrahedral FEM** (stable Neo-Hookean, Smith et al. 2018) — the volumetric solid that completes the deformable spectrum alongside cloth (shell), MPM (particle) and rod (filament). Forces are exactly `−∇energy` and the gradient of an outcome w.r.t. material stiffness is closed-form, both verified against finite differences. Plus **plasticity** and **viscoelasticity**. Near-incompressible silicone (ν → 0.4999) obeys the dilatational CFL bound, and a refinement study measured convergence-increment ratios of 0.4648 at ν = 0.30 against 0.4726 at ν = 0.499, so constant-strain tets do **not** lock measurably here — an earlier version of that note said they did. |
| [`ferromotion-dem`](https://crates.io/crates/ferromotion-dem) | **Discrete-element granular media** (sand, gravel, soil, pills) — spherical grains with a linear spring–dashpot normal law and a Coulomb-limited tangential spring (Cundall & Strack 1979). Verified on the invariants granular contact must obey: a frictionless head-on collision conserves momentum and kinetic energy, a resting grain settles to the analytic penetration `mg/kₙ`, and a poured pile comes to rest without exploding. |
| [`ferromotion-coupled`](https://crates.io/crates/ferromotion-coupled) | **Two-way multiphysics coupling** between the volumetric FEM solid and DEM grains — a symmetric spring–dashpot penalty applies equal-and-opposite force to both, so grains pile *on* a deforming body and a body sinks *into* grains. Because every internal and contact force is equal-and-opposite, total linear momentum is conserved exactly, which is the oracle a correct two-way coupling must pass. |
| [`ferromotion-circuit`](https://crates.io/crates/ferromotion-circuit) | **Electrical-network dynamics** by **Modified Nodal Analysis** — `G·x + C·ẋ = b(t)` integrated with the trapezoidal rule (SPICE's default), which is A-stable and maps an undamped `LC` pole exactly onto the unit circle, so a lossless tank conserves its energy to rounding. Nonlinear devices (Shockley diode, MOSFET-as-switch) make each step a Newton solve and digital logic expressible; `morpho` grows a netlist from one recursive rule, lowers it to transistors, and lets the analog node voltages decide whether the grown adder computes. |
...
| [`ferromotion-learn`](https://crates.io/crates/ferromotion-learn) | Differentiable, **physics-informed learning** — reverse-mode `Tape` and forward-mode `Dual` autodiff (both verified against finite differences) carrying **PINNs**, **Lagrangian / Hamiltonian nets** (DeLaN, HNN), **Neural ODEs**, **SINDy**, calibration, nonlinear least squares, and a **PPO** trainer with observation normalization. A physics prior enters guided (in the features), informed (in the loss) or encoded (in the architecture). No BLAS, no Python, no GPU required. |
...
| [`ferromotion-viz`](https://crates.io/crates/ferromotion-viz) | **Rerun logging** — robots, trajectories and calibration curves streamed to the [rerun.io](https://rerun.io) viewer through the standard entity-path/timeline model, saveable as `.rrd` for offline inspection. Deliberately a companion crate, so the model-based core stays dependency-light and wasm-clean. |

## Quickstart
```toml
[dependencies]
ferromotion-core = "0.72"
nalgebra = "0.35"
```
```rust
