// Run the batched-MPPI WebGPU rollout kernel in real Chrome (Metal) and report GPU/CPU match + timing.
const http = require("http");
const fs = require("fs");
const path = require("path");
const PW = require("/Users/dcharlot/vibe-coding/bmi-concept/ipai-tv/node_modules/playwright-core");

const html = fs.readFileSync(path.join(__dirname, "..", "gpu", "mppi.html"));
...
(async () => {
  ...
  await page.waitForFunction(() => window.__result !== undefined, { timeout: 40000 });
  const result = await page.evaluate(() => window.__result);
  console.log(JSON.stringify(result, null, 2));
  await browser.close();
  srv.close();
  process.exit(result && !result.error && result.maxRelErr < 1e-3 ? 0 : 1);
