...
The finding is sharper than "it conserves." A *tiny* non-conservativeness (0.041) drives a **runaway**: the invented energy accelerates the arm into larger errors, which invent more energy — the drift compounds without bound. Parameterizing the force as ∇V makes it curl-free **by construction**, at any fit error, and the runaway disappears. What remains for the cure is a **bounded** offset (it reads as 100% only because this light arm holds just 0.82 J). We assumed that offset was ordinary fit error that would shrink away with training, and measured it to check: training the potential net 12× longer improved its force fit **95×** (MSE 1.31e-5 → 1.38e-7) but moved the 5-second drift only 162% → 127%, still 70× above the exact-force reference's 1.8% — and the 3-second figure rose before it fell. So the residual is *not* simply fit error: past a point, accuracy stops buying conservation, and something else dominates (most likely the rollout leaving the region the net was trained on, where its fit says nothing). The structural claim is unaffected — the runaway is gone at any fit quality — but the leftover is a second, separate problem, not a convergence story. So conservativeness structure is *necessary* — it removes the catastrophic mode no integrator can fix — while the leftover drift is a distinct open problem that more accuracy does not solve. (The sweep above adds cosine learning-rate decay and takes the step count as an argument: `wm_so101_cure 480000`; the table's numbers are the original constant-rate run.) Remaining generalization: learn M(q) itself as a positive-definite net (a full Hamiltonian net); here M(q) is the known robot inertia — the same prior a free-form learned force was given, and still failed with, so the fix is the force parameterization.
