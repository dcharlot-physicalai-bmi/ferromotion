...
**3. Screen position is not physical position.** This one invalidates more work than the other two. A generated scene is 3D under perspective: an object receding along the floor climbs toward the horizon and shrinks, which in screen coordinates is *identical* to rising into the air. We watched a ball rest on the floor and then apparently ascend — energy from nothing, seemingly a flagrant violation. It was receding: its area fell 80% as it "climbed" (correlation +0.79 between screen height and size), and a monocular depth model over the same frames showed it moving steadily away. No violation, and no gravity reading either — because screen height conflates height with distance. Two cheap fixes we tried both failed, each caught by a sanity check worth copying: a ground-plane fit put the object *below the floor* in 51 of 72 frames (impossible), and a shadow-based estimator locked onto the floor edge instead of the shadow (its "shadow" sat at the same row every frame). A relative-depth model settles *whether* something violated physics. For a *metric* number we first concluded you need a reference object of known size or a calibrated camera, and that was too pessimistic — a round object carries its own ruler.

**The scale-free estimator.** Under perspective a sphere of true radius R at depth d has apparent radius r = fR/d, while its image offset from the principal point is v = fY/d. Divide one by the other and both the focal length and the depth cancel:

```
Y / R  =  v / r          height, measured in ball radii
```

No calibration, no depth model, no known object size, and — because the unknown depth cancels rather than being estimated — it is immune to the recession that defeats raw pixel tracking. Curvature is also unaffected by where you put the principal point, since guessing it wrong shifts only the linear term. On synthetic footage with a known answer it recovers gravity to five decimal places (89.18 vs 89.18 ball-radii/s², R² = 1.00000) and the true ball radius to three (0.110 m vs 0.11), *including a case where the ball recedes while falling*. Fit a parabola to Y/R against time and its curvature is g in ball-radii/s²; invert it and 9.81/g is the object size the depicted fall implies, which you can then sanity-check against what the object appears to be.

Turned on real generated video, it splits the question usefully. The **shape** of the fall replicates across seeds — clean parabolas, R² 0.992–0.995, which is what constant acceleration looks like. The **rate** does not: the same scene and prompt gave 3.16, 19.50 and −57.55 ball-radii/s² (the last accelerating *upward*), with a fourth seed producing no measurable fall. A world with real gravity has one value of it. Quote a rate from a single run at your peril — ours implied a six-metre ball and evaporated on replication.
